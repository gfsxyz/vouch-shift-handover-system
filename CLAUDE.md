...
(Sonnet 4.6, Zod, cached; a committed seed in `lib/extraction/seed/` lets it run keyless on the
sample). Windowing is [`lib/shift.ts`](lib/shift.ts) (tested first). Category/thread-key rules
